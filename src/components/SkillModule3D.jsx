...
function EquipmentMaterial({ active, accent, color = '#3a4447', metalness = 0.58, roughness = 0.34 }) {
  return (
    <meshStandardMaterial
      color={active ? '#596668' : color}
      metalness={metalness}
      roughness={roughness}
      emissive={accent}
      emissiveIntensity={active ? 0.12 : 0.008}
    />
  )
}
  ...
  return (
    <group>
      <RoundedBox args={[1.35, 2.05, 0.72]} radius={0.08} smoothness={4} position={[0, 1.03, 0]} castShadow>
        <EquipmentMaterial active={active} accent={accent} color="#3d474a" />
      </RoundedBox>
      <mesh position={[0, 1.42, 0.38]}>
        <boxGeometry args={[0.92, 0.55, 0.04]} />
        <meshStandardMaterial color="#172023" metalness={0.22} roughness={0.58} />
      </mesh>
      {[0.98, 0.72, 0.46].map((y, index) => (
        <group key={y}>
          <mesh position={[-0.31, y, 0.4]}>
            <boxGeometry args={[0.42, 0.17, 0.05]} />
            <meshStandardMaterial color="#273134" metalness={0.35} />
          </mesh>
      ...
      {[-0.42, -0.14, 0.14, 0.42].map((x) => (
        <mesh key={x} position={[x, 1.7, 0.415]}>
          <circleGeometry args={[0.025, 14]} />
          <meshBasicMaterial color={active && x === 0.14 ? accent : '#8a978f'} />
        </mesh>
      ))}
  ...
  return (
    <group>
      ...
      <mesh position={[0, 1.08, 0.01]}>
        ...
        <meshPhysicalMaterial color="#487180" transparent opacity={0.4} roughness={0.15} />
      </mesh>
      <mesh position={[0, 0.13, 0]}>
      ...
      <mesh position={[-0.48, 1.08, 0.54]}>
        <boxGeometry args={[0.07, 1.35, 0.04]} />
        <meshBasicMaterial color="#b5c2c1" transparent opacity={0.72} />
      </mesh>
  ...
  return (
    <group>
      <mesh position={[0, 0.55, 0]} rotation={[0, 0, Math.PI / 2]} castShadow>
        ...
        <EquipmentMaterial active={active} accent={accent} color="#4a5457" />
      </mesh>
      ...
      <RoundedBox args={[0.82, 1.18, 0.5]} radius={0.06} smoothness={4} position={[-0.82, 1.38, -0.12]} castShadow>
        <EquipmentMaterial active={active} accent={accent} color="#394447" />
      </RoundedBox>
      <mesh position={[-0.82, 1.58, 0.15]}>
        <planeGeometry args={[0.5, 0.28]} />
        <meshBasicMaterial color="#141d20" />
      </mesh>
      <mesh position={[-0.82, 1.58, 0.16]}>
        <planeGeometry args={[0.32, 0.08]} />
        <meshBasicMaterial color={active ? accent : '#617266'} />
      </mesh>
      {[[-1.02, 1.17], [-0.82, 1.17], [-0.62, 1.17]].map(([x, y], index) => (
        <mesh key={x} position={[x, y, 0.165]}>
          <circleGeometry args={[0.035, 16]} />
          <meshBasicMaterial color={index === 1 && active ? accent : '#8a9691'} />
        </mesh>
      ))}
  ...
  return (
    <group>
      <mesh position={[0, 0.62, 0]} rotation={[0, 0, Math.PI / 2]} castShadow>
        ...
        <EquipmentMaterial active={active} accent={accent} color="#596265" />
      </mesh>
      ...
      <RoundedBox args={[0.92, 0.62, 0.62]} radius={0.06} smoothness={4} position={[-0.95, 0.62, 0]} castShadow>
        <EquipmentMaterial active={active} accent={accent} color="#394346" />
      </RoundedBox>
  ...
  return (
    <group>
      <mesh position={[0, 0.22, 0]} castShadow>
        ...
        <EquipmentMaterial active={active} accent={accent} color="#465053" />
      </mesh>
      <mesh position={[0, 0.72, 0]} rotation={[0, 0, -0.28]} castShadow>
        <boxGeometry args={[0.46, 1.15, 0.46]} />
        <EquipmentMaterial active={active} accent={accent} color="#596366" />
      </mesh>
      ...
      <mesh position={[0.62, 1.63, 0]} rotation={[0, 0, -0.88]} castShadow>
        ...
        <EquipmentMaterial active={active} accent={accent} color="#677174" />
      </mesh>
...
function IndustrialMechanicsTools({ active, accent }) {
  return (
    <group>
      <RoundedBox args={[1.12, 1.55, 0.34]} radius={0.12} smoothness={5} position={[0, 0.85, 0]} rotation={[-0.18, 0, 0]} castShadow>
        <EquipmentMaterial active={active} accent={accent} color="#465154" roughness={0.44} />
      </RoundedBox>
      ...
      <mesh position={[-1.28, 0.15, -0.25]} rotation={[0, 0, -0.7]}>
        <boxGeometry args={[0.18, 1.25, 0.12]} />
        <meshStandardMaterial color="#778184" metalness={0.82} roughness={0.25} />
      </mesh>
      <mesh position={[-1.55, 0.57, -0.25]} rotation={[0, 0, -0.7]}>
        <torusGeometry args={[0.22, 0.08, 10, 24, Math.PI * 1.45]} />
        <meshStandardMaterial color="#8c9698" metalness={0.88} roughness={0.2} />
      </mesh>
    </group>
  )
}
...
const equipment = {
  ...
  fluid: FluidPower,
  robot: RobotArm,
  mechanics: IndustrialMechanicsTools,
}
...
export default function SkillModule3D({ module, selected, onNavigate }) {
  ...
  return (
    ...
    >
      ...
      <Html transform sprite center position={module.labelPosition} distanceFactor={8.5}>
        <button
